    ...
    render() {
        ...
        for (let l = 0; l < this.lights.length; l++) {
            ...
            for (let i = 0; i < this.meshes.length; i++) {
                ...
                let precomputeL_RGB = getRotationPrecomputeL(precomputeL[guiParams.envmapId], cameraModelMatrix);        
                ...
                for (let k in this.meshes[i].material.uniforms) {
                    if (k == 'uMoveWithCamera') { // The rotation of the skybox
                        gl.uniformMatrix4fv(
                            ...
                            false,
                            cameraModelMatrix);
                    } else if (k == 'uPrecomputeL_R') {
                        gl.uniformMatrix3fv(
                            this.meshes[i].shader.program.uniforms[k],
                            false,
                            precomputeL_RGB[0]);
                    } else if (k == 'uPrecomputeL_G') {
                        gl.uniformMatrix3fv(
                            this.meshes[i].shader.program.uniforms[k],
                            false,
                            precomputeL_RGB[1]);
                    } else if (k == 'uPrecomputeL_B') {
                        gl.uniformMatrix3fv(
                            this.meshes[i].shader.program.uniforms[k],
                            false,
                            precomputeL_RGB[2]);
